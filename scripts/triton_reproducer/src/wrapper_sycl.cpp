...
void tritonReproducer::readArguments(void) {
    auto path = std::getenv("INPATH");
    if (path)
        std::cout << "INPATH: " << path << std::endl;
    else
        std::cout << "INPATH environment variable is not set" << std::endl;
    this->dataDirPath = std::string(path);
    auto inFile= this->dataDirPath + "data/input.txt";
    std::ifstream inputFile(inFile); // Open the input file
...
T* tritonReproducer::setupBuffers(int vidx) {
    auto inFile = this->dataDirPath + std::string(this->argInfo[vidx + 2]);
    auto host_mem = this->file_ops<T>(inFile);
...
std::tuple<int, int> tritonReproducer::compareResults(void) {
    auto thFile = this->dataDirPath + "./data/th.bin";
    auto torch_output = this->file_ops<T>(thFile);
