...
struct KernelArguments {
  ...
  KernelArguments(const std::string &outtensorname) {
    ...
    tt_dumps_path = (env_path != nullptr)
                        ? env_path
                        : std::filesystem::current_path().string();
    if (std::filesystem::exists(tt_dumps_path)) {
      std::ifstream file(tt_dumps_path + "/args_data.json");
      if (!file.is_open()) {
    ...
    spv_name = tt_dumps_path + "/" + jsonData.at("spv_name").get<std::string>();
...
at::Tensor launchKernel(sycl::queue stream, sycl::kernel kernel,
  ...
  for (auto &item : triton_args.jsonData["argument_list"]) {
    if (item.contains("type")) {
      if (item.at("type").get<std::string>() == "tensor") {
        auto tensor_name = triton_args.tt_dumps_path + "/" +
...
int main(int argc, char **argv) {
  try {
    ...
    write_tensor(tritonArgDict.tt_dumps_path + "/cpp_outs.pt", output);
